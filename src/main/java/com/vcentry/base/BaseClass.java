...
public class BaseClass {
	...
	public static Logger log=org.apache.logging.log4j.LogManager.getLogger(BaseClass.class);
	...
	public static void waitForWebElement(By element) {
		try {
			...
			WebDriverWait wait=
					new WebDriverWait(driver, Duration.ofSeconds(Integer.parseInt(Function.getProp("wait"))));
		...
		public static void enterText(By element,String data) {
			try {
				...
				//e.printStackTrace();
				test.log(Status.FAIL,data + " is not entered in "+element);
				log.info(data + " is not entered in "+element);
			}
